...
def build_tokenizer(fnames, max_length, data_file):
    data_file = os.path.join(os.getcwd(), "datasets", data_file)
...
class Tokenizer(object):
    ...
    def from_files(cls, fnames, max_length, lower=True):
        corpus = set()
        for fname in fnames:
            fname = os.path.join(os.getcwd(), 'datasets', fname)
...
def build_embedding_matrix(vocab, embed_dim, data_file):
    data_file = os.path.join("datasets", data_file)
    if os.path.exists(data_file):
        ...
        fname = 'datasets/glove.twitter.27B.' + str(
            embed_dim) + 'd.txt'
